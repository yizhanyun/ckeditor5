...
The image resize feature is not enabled by default in any of the editor builds. In order to enable it, you need to load the {@link module:image/imageresize~ImageResize} plugin. Read more in the {@link features/installation Installation} section.
...
The `image_resized` class is used to disable `max-width` assigned by the {@link features/image-styles image styles} if one is applied to this image. For instance, the "side image" style is defined like this:
